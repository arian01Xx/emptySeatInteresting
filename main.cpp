...
public:
    int smallestChair(vector<vector<int>>& times, int targetFriend) {
        int n=times.size();
        vector<int> order(n); //numbers of chairs
        for(int i=0; i<n; ++i) order[i]=i;

        sort(order.begin(), order.end(), [&times](int a, int b){
            return times[a][0] < times[b][0]; //order from minur to mayor
        });
        ...
        for(int i: order){
            ...
            takenSeats.push({leave,seat}); //el momento en que se fue y el asiento
        }
        return -1;
    }
};
